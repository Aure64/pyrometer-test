...
const isEndorsementByDelegate = (operation, delegate) => {
    for (const contentsItem of operation.contents) {
        if ((contentsItem.kind === types_1.OpKind.ATTESTATION || contentsItem.kind === types_1.OpKind.ATTESTATION_WITH_DAL) &&
...
const checkBlockAccusationsForDoubleEndorsement = async (baker, operations) => {
    ...
    for (const operation of operations) {
        for (const contentsItem of operation.contents) {
            if (contentsItem.kind === types_1.OpKind.DOUBLE_ATTESTATION_EVIDENCE ||
                contentsItem.kind === types_1.OpKind.DOUBLE_PREATTESTATION_EVIDENCE) {
