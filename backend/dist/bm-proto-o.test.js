...
const bm_proto_o_1 = require("./bm-proto-o");
const block93416_1 = __importDefault(require("./testFixtures/o/block93416"));
const block93416rights_1 = __importDefault(require("./testFixtures/o/block93416rights"));
const block93415erights_1 = __importDefault(require("./testFixtures/o/block93415erights"));
const block93516_1 = __importDefault(require("./testFixtures/o/block93516"));
const block93516rights_1 = __importDefault(require("./testFixtures/o/block93516rights"));
const block93601_1 = __importDefault(require("./testFixtures/o/block93601"));
const block93601rights_1 = __importDefault(require("./testFixtures/o/block93601rights"));
const block96849_1 = __importDefault(require("./testFixtures/o/block96849"));
...
describe("checkBlockBakingRights", () => {
    it("returns baked event for block baked by baker with rights at round 0", () => {
        const block = block93416_1.default;
        const rights = block93416rights_1.default;
        expect(block.header.level).toEqual(rights[0].level);
        const delegate = rights[0].delegate;
        const result = (0, bm_proto_o_1.checkBlockBakingRights)({
            baker: delegate,
    ...
    it("returns missed bake for block baked by another baker", () => {
        const block = block93516_1.default;
        const rights = block93516rights_1.default;
        expect(block.header.level).toEqual(rights[0].level);
    ...
    it("returns null for a block that baker has rights for in a later round", () => {
        const block = block93601_1.default;
        const rights = block93601rights_1.default;
        expect(block.header.level).toEqual(rights[0].level);
        expect(rights[rights.length - 1].round).toEqual(block.header.payload_round + 1);
        const result = (0, bm_proto_o_1.checkBlockBakingRights)({
    ...
    it("returns missed bonus event for a block where baker and proposer are not the same", () => {
        const rights = block93516rights_1.default;
        const block = {
            ...block93516_1.default,
            metadata: { ...block93516_1.default.metadata, proposer: rights[0].delegate },
        };
        expect(block.header.level).toEqual(rights[0].level);
        expect(block.header.payload_round).toEqual(1);
        expect(block.metadata.baker).toEqual(rights[1].delegate);
...
describe("checkBlockEndorsingRights", () => {
    it("returns endorsed event when present in rights and endorsement was made", () => {
        ...
        expect(result).toEqual([events_1.Events.Endorsed, 780]);
    });
    it("returns missed endorsement event when present in rights but not in block operations", () => {
        ...
        expect(result).toEqual([events_1.Events.MissedEndorsement, 9]);
    });
    it("returns none when not in rights and not in block operations", () => {
        const block = block93416_1.default;
        const rights = [
            {
                ...block93415erights_1.default[0],
                delegates: [...block93415erights_1.default[0].delegates],
            },
        ];
        expect(block.header.level - 1).toEqual(rights[0].level);
...
describe("checkBlockAccusationsForDoubleBake", () => {
    it("returns true when block contains baker accusation", async () => {
        const block = block96849_1.default;
        const result = await (0, bm_proto_o_1.checkBlockAccusationsForDoubleBake)("tz3Q67aMz7gSMiQRcW729sXSfuMtkyAHYfqc", block.operations[2]);
        expect(result).toEqual(true);
    });
    it("returns false when a different baker is accused", async () => {
        const block = block96849_1.default;
        const result = await (0, bm_proto_o_1.checkBlockAccusationsForDoubleBake)("some other baker", block.operations[2]);
        expect(result).toEqual(false);
    });
    it("returns false when block contains accusation without metadata", async () => {
        const block = block96849_1.default;
        const evidence = block.operations[2][0];
        const contents0 = evidence.contents[0];
        ...
        const result = await (0, bm_proto_o_1.checkBlockAccusationsForDoubleBake)("tz3Q67aMz7gSMiQRcW729sXSfuMtkyAHYfqc", [evidenceNoMetadata]);
        expect(result).toEqual(false);
    });
    it("returns false when block contains no accusation", async () => {
        const block = block93416_1.default;
        expect(block.operations[2].length).toEqual(0);
        const result = await (0, bm_proto_o_1.checkBlockAccusationsForDoubleBake)(block.metadata.baker, block.operations[2]);
        expect(result).toEqual(false);
    });
});
describe("checkBlockAccusationsForDoubleEndorsement", () => {
    const evidence = {
        ...
        contents: [
            {
                ...
                op1: {
                    ...
                    signature: "sigkmyPhgZ2aMXDXcxycQN6CXmYG7ECgoDxbYAT1sfmJEqYSkvwf97ox8PzedJy6kRU5CvBdCzQGQgjp4N3y3MwqtAWgPAir",
                },
                op2: {
                    branch: "BLw6hXMmr3MGhWGhRymixheA6hr9MEV25raYxtFCZuH3fFEPwFH",
                    operations: {
                    ...
                    signature: "sigcpANnqCCwyW3DdmGTF1ZN4ihCx66yUrUiaEyuSPS9MMt1xGApwSpJQas7VNMQq1j7ZXZNF1bbWRB2y3okxaFHR4pX16ir",
                },
                slot: 1,
                metadata: {
                    balance_updates: [
                        {
                            kind: "freezer",
                            category: "deposits",
                            staker: { delegate: "tz1YPSCGWXwBdTncK2aCctSZAXWvGsGwVJqU" },
                            change: "-1920000000",
                            origin: "block",
                        },
                    ],
        ...
        signature: "sigW9mQYjwV3S38PzP1nTwDjRRUGj9857yoh5QRJwHWGyNR6CcS2jAaEXFHUg3UHr2idZ8Ywxddy2iZEva4Dztgo9GnPcCgA",
    };
    it("returns event if baker is accused", async () => {
        const result = await (0, bm_proto_o_1.checkBlockAccusationsForDoubleEndorsement)("tz1YPSCGWXwBdTncK2aCctSZAXWvGsGwVJqU", [evidence]);
        expect(result).toEqual(events_1.Events.DoubleEndorsed);
        const contents0 = evidence.contents[0];
        ...
        const result2 = await (0, bm_proto_o_1.checkBlockAccusationsForDoubleEndorsement)("tz1YPSCGWXwBdTncK2aCctSZAXWvGsGwVJqU", [doublepreEndorsementEvidence]);
        expect(result2).toEqual(events_1.Events.DoublePreendorsed);
    });
    it("returns null if different baker is accused", async () => {
        const result = await (0, bm_proto_o_1.checkBlockAccusationsForDoubleEndorsement)("some other baker", [evidence]);
    ...
    it("returns null if no metadata is included in accusation", async () => {
        ...
        const result = await (0, bm_proto_o_1.checkBlockAccusationsForDoubleEndorsement)("tz1YPSCGWXwBdTncK2aCctSZAXWvGsGwVJqU", [evidenceNoMetadata]);
        expect(result).toEqual(null);
    });
    it("returns null when there are no accusations", async () => {
        const result = await (0, bm_proto_o_1.checkBlockAccusationsForDoubleEndorsement)("some baker", []);
        expect(result).toEqual(null);
    });
});
