...
describe("format", () => {
    ...
    it("uses address aliases", async () => {
        expect(format.aggregateByBaker(events, true, true, false, {
            [baker2]: "baker 2",
            [baker5]: "baker 5",
...
`);
    ...
    it("just email subject if one line", async () => {
        ...
        expect(subject).toMatchInlineSnapshot(`"tz1irJKkXS2DBWkU1NnmFQx1c1L7pbGg4yhk 🥖 @1000[1-4]² 👍 @1000[2-5]² 😕 @10003"`);
        expect(text).toBe("");
    });
    ...
    it("order of events in summary is fixed", async () => {
        ...
        const events2 = (0, lodash_1.shuffle)(events);
        const events3 = (0, lodash_1.shuffle)(events);
