...
export default async ({
  ...
  const [bakingRights, endorsingRights] = <[BakingRightsQ, EndorsingRightsQ]>(
...
type CheckBlockEndorsingRightsArgs = {
  baker: string;
  endorsementOperations: OperationQ[] | OperationWithLegacyAttestationName[];
  level: number;
  endorsingRights: EndorsingRightsQ;
};
...
const isEndorsementByDelegate = (
  operation: OperationQ | OperationWithLegacyAttestationName,
  delegate: string,
): boolean => {
...
export const checkBlockAccusationsForDoubleEndorsement = async (
  baker: string,
  operations: OperationQ[] | OperationWithLegacyAttestationName[],
